...
_BUNDLED_ENFORCEMENT_ROOT = _RUNTIME_DIRECTORY / "_resources" / "enforcement"
# Installed hook runtimes are deliberately a single immutable directory, while
# the packaged application keeps canonical policy beneath ``_resources``.
if not _BUNDLED_ENFORCEMENT_ROOT.is_dir():
    _BUNDLED_ENFORCEMENT_ROOT = _RUNTIME_DIRECTORY
POLICY_PATH = _BUNDLED_ENFORCEMENT_ROOT / "command-policy.json"
STRUCTURED_POLICY_PATH = _BUNDLED_ENFORCEMENT_ROOT / "structured-tool-policy.json"
METADATA_PATH = _RUNTIME_DIRECTORY / "metadata.json"
if not METADATA_PATH.is_file():
    METADATA_PATH = _BUNDLED_ENFORCEMENT_ROOT / "runtime-metadata.example.json"
OPERATION_CLASSES = {
    "observe",
    "validate",
    "mutate",
...
def classify_target(
    ...
    if command is not None:
        ...
        for tokens in script.commands:
            ...
            if executable in {"ansible", "ansible-console", "ansible-playbook", "ansible-pull"}:
                inventory_names = {"--inventory", "--inventory-file", "-i"}
                inventories: list[str] = []
                arguments = tokens[1:]
                for index, token in enumerate(arguments):
                    lowered = token.lower()
                    if lowered in inventory_names and index + 1 < len(arguments):
                        inventories.append(arguments[index + 1])
                        continue
                    if any(lowered.startswith(name + "=") for name in inventory_names):
                        inventories.append(token.split("=", 1)[1])
                if len(inventories) > 1:
                    return None, None, "multiple Ansible inventory sources are protected"
                inventory = inventories[0] if inventories else None
                inventory_map = maps.get("ansible_inventories", {})
                if inventory and isinstance(inventory_map, Mapping) and inventory_map.get(inventory) in {
                    "dev",
                    "tst",
                    "int",
                    "prd",
                }:
                    return inventory, str(inventory_map[inventory]), "mapped Ansible inventory"
                return inventory, None, "Ansible inventory is not mapped"
