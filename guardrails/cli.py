...
from . import build, enforcement, install, packs, policy, routing, scan, state
...
def _resolve_consumer_products(value: str | None, home: Path, command: str) -> tuple[tuple[str, ...], dict[str, tuple[str, ...]]]:
    ...
    if command == "install":
        ...
        raise GuardrailsError(
            ...
            "python tools/guardrails.py install --product codex"
        )
...
def _run_consumer_install(
    ...
    *,
    ...
    with contextlib.redirect_stdout(details):
        install.prepare_installation(dry_run=args.dry_run)
        if updating:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "build":
            ...
            build.validate(selected_products(args.product))
            ...
            install.print_cursor_rules(clipboard=args.clipboard)
