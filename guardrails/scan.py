...
from .util import ROOT, GuardrailsError, atomic_write, home_path, json_bytes, read_json, sha256
...
def _generated_stale(repo: Path) -> list[Finding]:
    if repo.resolve(strict=False) != ROOT.resolve(strict=False):
        return []
    try:
        from . import build
...
def _changed_risk_matches(repo: Path) -> list[tuple[Path, str, str]]:
    config = read_json(ROOT / "risk/path-classification.json", default={})
