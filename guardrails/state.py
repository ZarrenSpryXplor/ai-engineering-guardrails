...
def load_state(home: Path) -> dict[str, Any]:
    ...
    if data.get("format_version") == 1:
        # Read legacy state for update/uninstall, but all new writes use v2.
        upgraded = empty_state()
        upgraded["source_digest"] = data.get("source_digest", "")
        upgraded["products"] = data.get("products", {})
        return upgraded
...
def source_digest() -> str:
    ...
    for path in sorted(set(source_paths), key=lambda value: value.relative_to(ROOT).as_posix()):
