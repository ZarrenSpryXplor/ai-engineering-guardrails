...
Organisation identities, labels, spaces, branches, accounts, regions, types, and thresholds live in synthetic fixture data and must be replaced. Build and tests never attach or deploy policies. `opa test platform-policies/spacelift` runs only when an existing `opa` executable is available; otherwise structural validation reports semantic execution skipped. This repository does not implement a Rego interpreter.
