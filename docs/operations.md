...
python tools/guardrails.py update --dry-run
python tools/guardrails.py update
```

Omitted product selection detects local executables, configuration directories, and managed state. Fresh installs select only detected products, install every stable capability pack as on-demand guidance, retain `routing_profile=none`, and use the non-mutating `infrastructure-observe` safety profile. Explicit product/profile/pack options remain available for advanced use. For diagnostics:

```sh
python tools/guardrails.py status --show-routing
...
```sh
python tools/guardrails.py status --product all --repo . --show-routing
python tools/guardrails.py effective --product all --repo .
python tools/guardrails.py diff-installed --product all
python tools/guardrails.py explain --command 'kubectl delete namespace example' --pack kubernetes
python tools/guardrails.py simulate --tool mcp__spacelift__query \
...
1. Run `python tools/guardrails.py print-cursor-rules`.
...
Audit is local JSONL under `~/.ai-guardrails/audit/`, rotated during writes at a bounded size. It records hashes and classifications only. `python tools/guardrails.py receipt --repo . --product all` produces a similarly content-free session summary.

## Repository scan and risk evidence

```sh
python tools/guardrails.py scan --repo . --format human
python tools/guardrails.py scan --repo . --format json --output guardrails.json
python tools/guardrails.py scan --repo . --format sarif --output guardrails.sarif
python tools/guardrails.py scan --repo . --format junit --output guardrails.xml
```
